...
A：所有可以执行的操作可以参考 [Python SDK 文档](/contests/THUAI7/python_sdk) 或 [C++ SDK 文档](/contests/THUAI7/cpp_sdk)。每 tick 内的操作按照先到先处理，譬如玩家在血量为 1 时，在同一 tick 内，服务端先收到对手攻击自己的操作，再收到使用急救包的操作，那么会死亡；反之如果服务端先收到使用急救包的操作则不会死亡。
