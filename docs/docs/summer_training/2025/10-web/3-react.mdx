...
Web 应用已经从简单的显示文字进化到展示图片等多媒体资源，乃至到如今复杂多样的高交互性内容展示。在这种进化的背后，是 Web 应用前端开发框架的发展。常用的框架有： React、Vue、Angular。
...
- 进入 create-react-app 创建的文件夹并安装 TypeScript 相关依赖
...
- 启动 React，在浏览器打开 `localhost:3000` 查看应用
...
React 对 TypeScript 语言作了一定的拓展，拓展后的 TSX 在拥有 TypeScript 特性的同时具有一些类似 HTML 的特征，其中最显著的特点就是标签语法的引入。
